  ...
  return [
    {
      title: '▶ ターンの進め方',
      items: [
        '自軍の労働者を動かしたいマスへ移動させる（動かさなくてもよい）',
        '動かし終えたら、円形の▶ボタン（マウスを近づけると「ターンを進める」と表示される）を押してターンを終了する',
        'ボタンを押すとCPU都市の労働者も自動で動き、両陣営の資源収穫・？マス判定が行われて次のターンに進む',
      ],
    },
    {
      title: '🧑‍🌾 労働者の移動',
    ...
    },
    {
      ...
      items: [
        'テックツリーの全15技術を解放する',
        `食料・生産力・金の合計が、難易度ごとのしきい値（${thresholdList}）に到達する。現在の難易度：${currentLabel}（「金融」を解放するとさらに20%下がる）`,
      ],
    },
  ]
}
...
export function renderRulesModal(currentDifficulty) {
  return `
    <div id="rules-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div class="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-4 shadow-xl">
        ...
        ${buildSections(currentDifficulty).map(renderSection).join('')}
      </div>
    </div>
  `
}
